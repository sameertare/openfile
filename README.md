...
- **One loaded dataset:** load your own games or an opponent's the same way — whichever account you load becomes the tree and the drill deck. No separate "profiles" to switch between; loading a new source replaces what was there.
- **Input:** drop PGN file(s), try the bundled sample, load a previously-saved `.tree.json` (below), or fetch an account's games directly by username — from **lichess** (`GET lichess.org/api/games/user/:username`, streamed as PGN) or **chess.com** (its public "Published Data API": fetch the account's monthly archive list, then the N most recent months in parallel, concatenating each game's own `pgn` field; its `[Site]` header is never a real URL, so the game's separate `url` field is injected as a `[Link]` header so the games-list "View" link still resolves). Neither needs auth for public games. Without a forced username (plain PGN upload), the main player is auto-detected the same way Performance Analysis does it (most frequent name across games, with casing/"Last, First"/nickname variants folded together); a single-account username fetch skips that heuristic and attributes every game to that account directly.
- **🔗 Combine lichess + chess.com into one tree:** a toggle above the two fetch sections. Off (default): each fetch button works independently exactly as described above — fetching a second, differently-named account after the first would silently narrow `matchKeys` to just the new one, so don't mix sources this way if you want them combined; that's what the toggle is for. On: swaps in a single "Build combined tree" button that fetches both usernames (one, or both, as filled in) and unions their `matchKeys`, so a game under *either* identity counts toward one tree — for a player with different handles on each site. The combined identity displays as `"handleA + handleB"`, and a shareable link (`?src=combined&lichessUser=...&chesscomUser=...`) round-trips it the same way a single-source link does.
- **Tree:** built entirely client-side (`src/openingTree.ts`), transposition-aware — nodes are keyed by position (board/turn/castling/en-passant), not move sequence, so the same position reached via different move orders merges into one node. Every node tracks games/wins/draws/losses reached through it, average opponent rating, an estimated FIDE-style performance rating, and a reference to every underlying game that passed through.
- **Browsing:** a board (reusing the same `Board` component as Game Analysis) plus a clickable breadcrumb and a move-list table sorted by frequency, each row showing games played, score %, avg opponent rating, and a win/draw/loss bar. Click a move to drill in; **Back**/**Start**/flip to navigate.
...
- **📋 Full scouting report — companion page (`/scouting-report.html`):** deliberately not a tenth hub tool or sidebar link — it has no load UI of its own, and is only reachable via the "📋 Full scouting report →" button that appears once games are loaded here. Clicking it hands off the loaded games to the companion page via a one-time `sessionStorage` payload (`src/scoutingReport.ts` reads and immediately clears it), which then renders overall W-D-L/score, by-color breakdown, results by time control, and most-played/best-scoring/worst-scoring openings. Deliberately skips per-move engine analysis (too slow across a whole account's games); reuses the same `src/aggregate.ts` openings/time-control logic Performance Analysis uses, just without the sections that need engine evals (phases, tactics, patterns, recommendations). Visiting the page directly (no handoff pending) shows an empty state pointing back here.
- **🎓 Drill your repertoire:** below the tree, for whatever's loaded. Quizzes you on your own turn only, at every position in the tree where you've actually played a move — never on the opponent's replies. Click a piece to answer; correct means the move you played is *any* move you've actually played from that position before (a repertoire can branch — sometimes you play e4, sometimes d4, both count), not just the single most common one. A miss shows every move you've actually played there, with games/score, so you can see what you meant to play. Spaced repetition (`src/srs.ts`, a simplified SM-2 — binary correct/incorrect instead of SM-2's 0–5 quality scale, everything else the same shape: interval, ease factor, due date) schedules each position further out after a correct answer and back to daily after a miss, so weak branches surface more often without re-drilling everything you already know. A missed position also gets one more attempt later in the same session, on top of its rescheduled due date. Progress persists to `localStorage`, keyed per player *and* color. "Start drilling" pulls in up to 30 due (or new) positions per session, oldest-due/never-seen first.

Not in v1 (noted as future work): variant support and master-game database comparison — real openingtree.com/chesscope.com features, scoped out to keep this focused.
