...
- **Pairing engine:** Dutch-style fold pairing — round 1 pairs the top half vs the bottom half by rating; later rounds pair within score groups, down-float odd players, avoid rematches *and* marked family/sibling conflicts (each with a global fallback that relaxes automatically if a strict, conflict-free pairing isn't possible), balance colors, and assign a full-point bye to the lowest player who hasn't had one when the field (after bye requests) is odd.
- **"Why this pairing?"** — a small ⓘ button next to every board (and bye) explains the match-up in plain English, SwissSys-style: each player's score entering the round (same score group vs. one having floated), the color-balance reasoning behind who got White (who was "owed" it, or how alternation/seed order broke a tie), whether it's a rematch or first meeting, and — if it applies — a note that the two are a marked family group paired only because no conflict-free alternative existed. Works on any round, not just the latest, and is reconstructed from the round-by-round history rather than the pairing algorithm's internal trace, so it reports the objective facts of the match-up rather than every tiebreak the algorithm weighed.
